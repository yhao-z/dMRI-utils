function [range,varargout] = auto_para_range(f,param,varargin)
% 简单函数测试
...
% f(z)
% para_range = auto_para_range(f,z);
% [metrics,X1,X2] = auto_para_traverse(f,z,para_range,[1 1]);
%
% 记录问题
% 1. step必须等于1
% 2. [done] paras_all 应该是把X1,X2.....这些输出出来才行，否则不大好用，最好弄成varargout
% 3. para和param乱用，应该把名称统一 
% 4. [done] range输出一个大区间
% 5. [done] 有时候可能0才是最优解，这样的话就会导致一直循环，这怎么办？
%

disp('[start auto_para_range ... ]')
...
range = NaN(len_param,2); 
opt_para = NaN(len_param,1);
...
for ii = 1:len_param
    str1 = [str1,',','length(unique(axises(:,',num2str(ii),')))'];
    str_temp = str_sub;
    str_temp(2*ii-1) = ':';
    str2 = [str2,',','ismember(X',num2str(ii),'(',str_temp,'),X',num2str(ii),'_prev(',str_temp,')) == 1']; %X1(:,1)-Min(1)+1,X2(1,:)-Min(2)+1,X3(1,:)-Min(3)+1
    str3 = [str3,',','sort(unique(axises(:,',num2str(ii),')))'];
    str4 = [str4,',','X',num2str(ii)];
    str5 = [str5,'X',num2str(ii),'_prev = X',num2str(ii),';'];
    str6 = [str6,',','ismember(X',num2str(ii),'(',str_temp,'),axises1(:,',num2str(ii),')) == 1']; %X1(:,1)-Min(1)+1,X2(1,:)-Min(2)+1,X3(1,:)-Min(3)+1
    str7 = [str7,',','X',num2str(ii)];
...
while(1)
    ...
    [range(i,:),opt_para(i),paras_all1,metrics_all1,strange] = auto_param_1d(f,param,param_fields,i,log_or_normal,I_log_or_normal,step,strange); % 搜索某一维度
    param.(param_fields{i}) = I_log_or_normal(opt_para(i)); % 将最大值赋给参数结构体，更新它
    ...
    if iter == 1
        ...
        metrics_all = NaN(size(X1));
    elseif iter ~= 1 % 生成新的较大的《搜索记录张量》，并将之前的搜索结果填充进去
        eval(str5);
        eval(['[',str4,'] = ndgrid(',str3,');']); % 生成《搜索记录坐标张量》
        eval(['temp = NaN(size(X1));'])
        eval(['temp(',str2,') = metrics_all;'])
        metrics_all = temp;
    end
    eval(['metrics_all(',str6,') = metrics_all1;']) % 将新的搜索结果填充到《搜索记录张量》中
    ...
    if iter == 1 % 第一次迭代不考虑，直接开始第二次
        i = i+1;
        maxi = max(maxi,i);
    elseif opt_para(i) == param_fix(i) % 当前迭代维得到的表现最好的参数与之前搜索其它维度时使用的该维度的固定参数一致
...
varargout = eval(['{opt_para,',str7,',metrics_all}']);

% 作图，但是也只能画1 2 3维，再高画不了了
if len_param == 1
    figure('Name','[auto_para_range]');
...
disp('[End auto_para_range]')

if ~isempty(strange)
    disp(['》》》》》》》》》》》》 [NOTE][strange] the opt of the param (',num2str(unique(strange)),') seems to be 0'])
end
%% 1d
function [range,opt_para,paras_all,metrics_all,strange] = auto_param_1d(f,param,param_fields,param_order,log_or_normal,I_log_or_normal,step,strange)
...
while(1)
    ...
    if ismember(i, [1 2]) % 第一次第二次不好比较，不知道应该往前还是往后，因此默认往前（+）
        ...
        [a,I] = sort(a); % 排下序,升序，最小的在第一个
        metric = metric(I);
        [~,index] = max(metric);% 表现最好
        metric_nomax = metric;% 去掉表现最好，为了找表现次好
        metric_nomax(index) = min(metric(:));
        if index == i % 表现最好的在两个端点，那需要继续找
            a(i+1) = a(i)+step;
        elseif index == 1
            % 为了SNR指标故意设计的，所以说不太能推广到所有metric
            if abs(metric(1)-metric(2)) < 1e-4 && abs(metric(2)-metric(3)) < 1e-4 && a(1) < -4
                range = [a(index) a(index+2)];
                opt_para = a(index);
                paras_all = a;
                metrics_all = metric;
                disp(['**** the opt of the ',num2str(param_order),'th param seems to be 0 ****'])
                strange(end+1) = param_order;
                break;
            end
            a(i+1) = a(1)-step;
        else % 不在端点，看看能不能停止
